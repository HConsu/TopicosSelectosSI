...
begin
  if DBGrid1.Fields[0].AsString<>'' then
     begin
    if application.MessageBox (pchar('¿Esta seguro que desea reactivar esta suscripción?'), pchar('Confirmar reactivación'), (MB_YESNO + MB_ICONQUESTION)) = IDYES then
           begin
             FPrincipalCirculacion.quienllamanueva:=2;
               ShowMessage('Se abrirá la ventana de nueva suscripcion, para terminar la reinserción de la suscripción en la ruta, si cancela el proceso, la suscripción no se reactivara y no aparecerá en ruta de entrega.');
