...
begin
  ...
  { ZQtmp.Close;
   ...
   ZQtmp.ExecSQL;}
   ShowMessage('El subscriptor fue dado de dalta en el área indicada, verifique el orden de reparto');
   agregar.Enabled := FALSE;
   agregararea.Enabled:=FALSE;
   ZQAreaGrid.Refresh;
   DBGridArea.Refresh;
...
begin
  if FPrincipalCirculacion.quienllamaareadeentrega = 1 then
    ...
    ZQAreaGrid.SQL.Add('Select Orden_en_reparto AS "Orden de entrega",NoSubscripcion,Area as "Área",no_ruta AS "Ruta"  from t_descripcion_ruta_edificio where id_edificio='+numero_de_edificio+' order by  Orden_en_reparto');
    ZQAreaGrid.ExecSQL;
    ZQAreaGrid.Open;
    ZQArea.Close;
    ZQArea.SQL.Clear;
