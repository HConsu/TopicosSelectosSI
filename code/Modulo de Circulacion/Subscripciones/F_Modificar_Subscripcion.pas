...
begin
   if application.MessageBox(pchar ('¿Esta seguro que quiere actualizar los datos de la subscripcion?'), pchar('Confirmar actualización'), (MB_YESNO + MB_ICONQUESTION)) = IDYES then
            begin
...
{         if ((combo_edificio.Text<>'') AND (calle.Text<>'')) then
       ...
       {---------------------------------------------------------------------------------------}
        ...
        ZQb.Close;
        ZQb.SQL.Clear;
        ZQb.SQL.Add('Insert into b_subscripciones (Descripcion,usuario) VALUES("Se modifico la subscripcion '+nosusbcripcion.Text+' atencíon a:'+atenciona.Text+',Costo:'+costo.Text+',Colonia:'+colonia.Text+',calle:'+calle.Text+',ruta:'+ruta.Text+'","Jefe de circulacion")');
        ZQb.ExecSQL;
        ZQb.Close;
    {------------Bitacora-----------------------------------------}

       ShowMessage('Se actualizaron los datos de la subscripción');
...
begin
if application.MessageBox (pchar('¿Esta seguro que desea actualizar los datos de la subscripcion?'), pchar('Confirmar actualización'), (MB_YESNO + MB_ICONQUESTION)) = IDYES then
           begin
...
//          if ((rutaactual<>ruta.KeyValue) OR (calle.Text<>callecontrol)) then
 //         begin
     ...
     {------------Bitacora-----------------------------------------}
              ZQb.Close;
              ZQb.SQL.Clear;
              ZQb.SQL.Add('Insert into b_subscripciones (Descripcion,usuario) VALUES("Se modifico la subscripcion '+nosusbcripcion.Text+' atencíon a:'+atenciona.Text+',Costo:'+costo.Text+',Edificio:'+combo_edificio.Text+'","Jefe de circulacion")');
              ZQb.ExecSQL;
              ZQb.Close;
     {------------Bitacora-----------------------------------------}
